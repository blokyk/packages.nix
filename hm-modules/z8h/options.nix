...
in {
  options = {
    programs.z8h = {
      enable = mkEnableOption "z8h, a turnkey configuration for zsh based on zsh4humans";
      ...
      #   # this should be doable directly with mkKeybindings's `keyMapper`
      #   macos-option-as-alt = mkOption {
      #     type = bool;
      #     default = true;
      #     description = ''
      #       Whether the mac keyboard's Option key should be equivalent to Alt in keybindings.
      #     '';
      #   };
      # };
  ...
  config = {
    assertions = [
      # {
      #   assertion = cfg.prompt-at-bottom -> (cfg.start-tmux != "no");
      #   message = "The z4h option `prompt-at-bottom` has no effect if `start-tmux` is disabled, but it is currently set to '${toString cfg.start-tmux}'";
      # }

      # # make sure that `programs.z8h.terminal.title` only has `local.foo`, `ssh.foo`, or `*.foo`, and nothing else
      # (let
      ...
      #     builtins.filter
      #       (x: !(isValid x))
      #       options.programs.z8h.terminal.title.definitionsWithLocations;
      # in {
      #   assertion = builtins.all isValid shells;
      #   message =
      #     "The z8h option `terminal.title` can only be set for either `local` or `ssh` shell types, but some were not: ${
